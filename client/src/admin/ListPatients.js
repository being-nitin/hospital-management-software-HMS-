...
const ListPatients = () => {
  ...
  const { loading, error, patients: { patient, totalPages } = {} } = patientList;
  ...
  const menu = (patientId) => (
      <Menu>
        ...
        <Menu.Item key="2">
          <Link to={`/add-appointment/${patientId}`}>Add Appointment</Link>
        </Menu.Item>
        <Menu.Item key="3">
      ...
      <div className="row mb-3">
        ...
        <div className="col-md-2">
          <input
            ...
            onChange={(e) =>
              setSearchDetails({ ...searchDetails, phoneNo: e.target.value })
            }
          />
        </div>
        <div className="col-md-2">
        ...
        <div className="col-md-2">
          <input
            ...
            onChange={(e) =>
              setSearchDetails({ ...searchDetails, address: e.target.value })
            }
          />
        </div>
        <div className="col-md-2">
              ...
              {patient &&
                patient.map((p, i) => (
                  <tr key={i}>
                    ...
                    <td>{p.phoneNo}</td>
                    <td>{moment(p.regDate).format("YYYY-MM-DD HH:mm Z")}</td>
                    <td>{p.gender}</td>
                    <td>
                      <Dropdown overlay={menu(p._id)} trigger={['click']}>
                        <Button>
